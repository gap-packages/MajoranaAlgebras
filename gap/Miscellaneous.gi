...
InstallGlobalFunction( MAJORANA_ConvertToBasis,
  ...
  InstallGlobalFunction( MAJORANA_FindTauMapEigenspace,

    function( axis, basis, plus_evals, minus_evals, rep)

    local dim, field, adj, espace, ev, tau, u, i, j;

    dim := Nrows(basis);
    field := rep.field;
    adj := MAJORANA_AdjointAction(axis, basis, rep);

    espace := SparseMatrix(0, dim, [], [], field);
    tau := SparseMatrix(0, dim, [], [], field);

    for ev in Concatenation(plus_evals, minus_evals) do
        espace := UnionOfRows(espace, KernelMat(adj - ev*SparseIdentityMatrix(dim, field)).relations);
    od;

    for i in [1..dim] do
        u := SparseMatrix(1, dim, [[i]], [[One(field)]], field);
        u := MAJORANA_ConvertToBasis(espace, u);

        for j in [Size(plus_evals) + 1 .. dim] do
            SetEntry(u, 1, j, -GetEntry(u, 1, j));
        od;

        tau := UnionOfRows(tau, u*espace);
    od;

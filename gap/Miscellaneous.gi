...
InstallGlobalFunction( MAJORANA_Eigenvectors,
    ...
    if not IsBound(rep.evecs[i].(String(eval))) then
        if eval = 1 then
            return SparseMatrix( 1, Size(rep.setup.coords), [ [ index ] ], [ [ 1 ] ], rep.field);
        else
            return SparseMatrix( 0, Size(rep.setup.coords), [], [], rep.field);
        fi;
    fi;

    # Conjugate the eigevectors of the orbit rep by g
    evecs := SparseMatrix( 0, Size(rep.setup.coords), [], [], rep.field);
...
InstallGlobalFunction( MAJORANA_Basis,
    ...
    basis := SparseMatrix( 0, dim, [], [], rep.field);

    for i in [1..dim] do
        if rep.setup.nullspace.heads[i] = 0 then
            basis := UnionOfRows(basis, SparseMatrix(1, dim, [[i]], [[1]], rep.field));
        fi;
    od;

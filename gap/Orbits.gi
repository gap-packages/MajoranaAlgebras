            ...
            if SetUp.pairorbit[i][j] = 0 then 
                ...
                for p in orb do 
                    ...
                    for gen in gens do 
                        ...
                        if OnPairs(SetUp.coords{SetUp.pairreps[y]},g) <> q then 
                            Error("setup 0");
                        fi;
                        ...
                        if not q in orb then 
                        
                            Add( orb, q );
                            Add( elts, g);
                            
                            MAJORANA_AddPowers(q, y, g, SetUp);
    ...
    for i in [1 .. dim] do 
        ...
        for j in [Maximum(t, i) + 1 .. dim] do 
            if Order(SetUp.coords[j]) = 5 then 
                ...
                if o = 5 then 
                    if SetUp.coords[i] in [q[1]^2,q[1]^3] then 
                        sign := -sign;
                    elif not SetUp.coords[i] in [q[1],q[1]^4] then
                        Error("setup 1");
                    fi;
                fi;
                ...
                if SetUp.coords[j] in [q[2]^2,q[2]^3] then 
                    sign := -sign;
                elif not SetUp.coords[j] in [q[2],q[2]^4] then
                    Error("setup 2");
                fi;  
...
InstallGlobalFunction( MAJORANA_AddPowers,
    ...
    pos_1 := Position(SetUp.longcoords, pnt[1]);
    ...
    pos_2 := Position(SetUp.longcoords, pnt[2]);
